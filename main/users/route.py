...
from main.users.form import RegistrationForm, LoginForm, UpdateAccountForm, ResetPasswordForm, RequestResetForm
...
def account():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('your acct has been updated succesfully,', 'success')
...
@users.route("/reset_request/<token>", methods=['POST', 'GET'])
def reset_token(token):
    if current_user.is_authenticated:
        return redirect('/home')
    user = User.verify_reset_token(token)
    if user is None:
        flash('In valid email or expired token, warning')
